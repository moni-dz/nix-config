final: prev: {
  discord-canary = prev.discord-canary.overrideAttrs (old: rec {
    powercord = prev.stdenv.mkDerivation rec {
      ...
      buildPhase = "${prev.nodePackages.npm}/bin/npm i"; # FIXME: do I disable nix.useSandbox?

      installPhase = "substituteInPlace injectors/linux.js --replace /opt/DiscordCanary ${prev.discord-canary}/opt/DiscordCanary";
