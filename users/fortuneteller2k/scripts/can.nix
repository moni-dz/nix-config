  ...
  can() {
    ...
    case "$1" in
      ...
      switch)
        ...
        [ "$?" -eq 0 ] && /run/current-system/sw/bin/ls --color=auto -1 /nix/var/nix/profiles | tail -n 2 | awk '{print "/nix/var/nix/profiles/" $0}' - | xargs nvd diff
        [ "$?" -eq 0 ] && /run/current-system/sw/bin/ls --color=auto -1 /nix/var/nix/profiles/per-user/fortuneteller2k | tail -n 4 | head -n 2 | awk '{print "/nix/var/nix/profiles/per-user/fortuneteller2k/" $0}' - | xargs nvd diff
        ;;
      rollback)
